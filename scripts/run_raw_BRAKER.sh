...
# run BRAKER3
#--prot_seq=${ORTHODB}
srun -c ${T} --mem 500G -p amd_1Tb --time 30-0 singularity exec -B "$(dirname ${AUGUSTUS_CONFIG_PATH}),$(dirname ${OUT_DIR}),$(dirname ${GENOME}),$(dirname ${ORTHODB}),$(dirname ${BAM}),/beegfs/scratch/ws/ws1/tsoies-DL/final_fastq" braker3.sif braker.pl --workingdir=${OUT_DIR} \
    --overwrite --genome=${GENOME} --species=$(basename ${OUT_DIR}) \
    --threads ${T} \
    --rnaseq_sets_dir=/beegfs/scratch/ws/ws1/tsoies-DL/final_fastq \
    --rnaseq_sets_ids=$(basename ${OUT_DIR}) \
